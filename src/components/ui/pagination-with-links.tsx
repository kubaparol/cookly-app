...
export function PaginationWithLinks({
  ...
  const buildLink = useCallback(
    (newPage: number) => {
      // If pathPattern is provided, use it for path-based pagination
      if (pathPattern) {
        const basePath = pathPattern.replace(':page', String(newPage));
        const queryString = searchParams?.toString();
        return queryString ? `${basePath}?${queryString}` : basePath;
      }

      // Otherwise use query parameter-based pagination
    ...
    [pageSearchParam, searchParams, pathname, pathPattern],
  );
  ...
  const navToPageSize = useCallback(
    (newPageSize: number) => {
      ...
      if (!pathPattern) {
        // For query parameter-based pagination, reset the page
        newSearchParams.delete(pageSearchParam || 'page');
        router.push(`${pathname}?${newSearchParams.toString()}`);
      } else {
        // For path-based pagination, navigate to page 1 with new page size
        const basePath = pathPattern.replace(':page', '1');
